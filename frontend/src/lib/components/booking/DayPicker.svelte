...
<div class="space-y-1.5 {className}">
	...
	<Select.Root type="single" bind:value={selectedDate}>
		<Select.Trigger
			...
			class="flex h-12 w-full items-center justify-between rounded-brutalist-md border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-4 py-2 text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-0 focus:border-indigo-500"
		>
		...
		<Select.Portal>
			<Select.Content
				class="z-50 max-h-60 min-w-[8rem] overflow-auto rounded-brutalist-md border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-md animate-in fade-in-0 zoom-in-95"
				sideOffset={4}
			>
				<Select.Viewport class="p-1">
					{#each dateOptions as option (option.value)}
						<Select.Item
							value={option.value}
							label={option.label}
							class="relative flex cursor-pointer select-none items-center rounded-brutalist-sm py-3 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition-colors hover:bg-gray-100 dark:hover:bg-neutral-800 focus:bg-gray-100 dark:focus:bg-neutral-800 data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
						>
