	...
	<div class="flex flex-col gap-3 sm:gap-4">
		...
		<div class="flex items-start gap-3">
			<div class="flex-shrink-0 text-2xl" aria-hidden="true">
				{#if option.type === 3}
					<!-- Multi-day icon -->
					<svg class="w-5 h-5 sm:w-6 sm:h-6 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			<div class="flex-1 min-w-0">
				<p class="text-sm sm:text-base font-medium text-text-primary">{formattedDate}</p>
				<p class="text-xs sm:text-sm text-text-secondary">{formattedTime}</p>
			</div>
		</div>
