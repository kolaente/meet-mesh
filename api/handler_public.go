...
func (h *Handler) GetPublicLink(ctx context.Context, params gen.GetPublicLinkParams) (gen.GetPublicLinkRes, error) {
	var link Link
	if err := h.db.Preload("Slots").Where("slug = ? AND status = ?", params.Slug, LinkStatusActive).First(&link).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return &gen.Error{Message: "Link not found"}, nil
		}
		return nil, err
	}
...
func (h *Handler) GetAvailability(ctx context.Context, params gen.GetAvailabilityParams) (*gen.GetAvailabilityOK, error) {
	...
	if link.Type != LinkTypeBooking {
		return nil, &ogenerrors.DecodeBodyError{
			...
			Body:        nil,
			Err:         errors.New("availability only for booking links"),
		}
	}
...
func (h *Handler) CreateBooking(ctx context.Context, req *gen.CreateBookingReq, params gen.CreateBookingParams) (gen.CreateBookingRes, error) {
	...
	if link.Type != LinkTypeBooking {
		return &gen.Error{Message: "Cannot book on poll link"}, nil
	}
